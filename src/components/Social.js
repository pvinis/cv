import React from 'react'
import { library } from '@fortawesome/fontawesome-svg-core'
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome'
import { fab } from '@fortawesome/free-brands-svg-icons'
import { fas } from '@fortawesome/free-solid-svg-icons'

import {
	Text,
	View,
} from '.'
import { Colors } from '../ui'
import { FlatList } from 'react-native-web'

library.add(fab, fas)


const SocialInfo = {
	phone: {
		link: ({number}) => `tel: ${number}`,
		icon: 'mobile-alt',
	},
	github: {
		link: ({username}) => `https://github.com/${username}`,
		icon: ['fab', 'github'],
	},
	gitlab: {
		link: ({username}) => `https://gitlab.com/${username}`,
		icon: ['fab', 'gitlab'],
	},
	stackoverflow: {
		link: ({id, name}) => `https://stackoverflow.com/users/${id}/${name}`,
		icon: ['fab', 'stack-overflow'],
	},
	linkedin: {
		link: ({username}) => `https://linkedin.com/in/${username}`,
		icon: ['fab', 'linkedin'],
	},
	ghuser: {
		link: ({username}) => `https://ghuser.io/${username}`,
		icon: ['fab', 'github-alt'],
	},
	website: {
		link: ({url}) => url,
		icon: 'globe',
	},
	//email,
	skype: {
		link: ({url}) => url,
		icon: ['fab', 'skype'],
	},
}


const Social = props => (
	<View style={{alignItems: 'center'}}>
		<FlatList
			horizontal
			data={['phone', 'ghuser', 'github', 'gitlab', 'linkedin', 'stackoverflow', 'website', 'skype']}
			renderItem={(({ item }) => {
				const { link, icon } = SocialInfo[item]
				return (
					<a
						key={item}
						href={link(props.links[item])}
						style={{
							textDecoration: 'none',
							color: Colors.text,
						}}
					>
						<Text>
							<FontAwesomeIcon icon={icon} />
							{' '}
							{props.links[item].display || props.links[item].username}
						</Text>
					</a>
				)
			})}
			ItemSeparatorComponent={() => <Text> | </Text>}
		/>
	</View>
)

export default Social
